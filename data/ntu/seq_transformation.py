...
#### comment this, if you don't want to process in chunk

# def split_dataset(skes_joints, label, performer, camera, evaluation, save_path, chunk_size=1000):
#     train_indices, test_indices = get_indices(performer, camera, evaluation)
#     num_train_chunks = len(train_indices) // chunk_size + 1
#     num_test_chunks = len(test_indices) // chunk_size + 1

#     for i in range(num_train_chunks):
#         start = i * chunk_size
#         end = min((i + 1) * chunk_size, len(train_indices))
#         train_chunk_indices = train_indices[start:end]
#         train_labels = label[train_chunk_indices]
#         train_x = skes_joints[train_chunk_indices]
#         train_y = one_hot_vector(train_labels)
#         save_name = 'NTU60_%s_train_chunk_%d.npz' % (evaluation, i)
#         np.savez(osp.join(save_path, save_name), x_train=train_x, y_train=train_y)

#     for i in range(num_test_chunks):
#         start = i * chunk_size
#         end = min((i + 1) * chunk_size, len(test_indices))
#         test_chunk_indices = test_indices[start:end]
#         test_labels = label[test_chunk_indices]
#         test_x = skes_joints[test_chunk_indices]
#         test_y = one_hot_vector(test_labels)
#         save_name = 'NTU60_%s_test_chunk_%d.npz' % (evaluation, i)
#         np.savez(osp.join(save_path, save_name), x_test=test_x, y_test=test_y)
# def concat_npz_files(save_path, evaluation, dataset_type):
#     """
#     Concatenate npz files saved in chunks.
...
#     Returns:
#         x (np.ndarray): The concatenated x data.
#         y (np.ndarray): The concatenated y data.
#     """
#     x_list = []
#     y_list = []
...
#     i = 0
#     while True:
#         file_name = 'NTU60_%s_%s_chunk_%d.npz' % (evaluation, dataset_type, i)
...
#### end of "comment this, if you don't want to process in chunk"
...
if __name__ == '__main__':
    ...
    for evaluation in evaluations:
        ...
        # comment this, if you dont want to process in chunk

        # train_x,train_y=concat_npz_files(save_path, evaluation, 'train')
        # test_x, test_y =concat_npz_files(save_path, evaluation, 'test')
        # # Concatenate the training and testing data
        # x = np.concatenate([train_x, test_x], axis=0)
        # y = np.concatenate([train_y, test_y], axis=0)
        # # Save the concatenated data
        # np.savez(osp.join(save_path, 'NTU60_%s.npz' % evaluation), x=x, y=y)
        # # Delete the non-concatennated data
        # delete_chunk_files(save_path, evaluation, 'train')
        # delete_chunk_files(save_path, evaluation, 'test')

        # end of "comment this, if you dont want to process in chunk"
